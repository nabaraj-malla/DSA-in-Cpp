...
using namespace std;
bool BST_helper(BinaryTreeNode<int> *root, int min, int max)
{
    if (root == NULL)
    {
        return true;
    ...
    bool isLeftOk = BST_helper(root->left, min, root->data - 1);
    bool isRightOk = BST_helper(root->right, root->data, max);
